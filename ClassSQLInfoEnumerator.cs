...
{
    /// <summary>
    /// Summary Descriptionfor SQLInfoEnumerator.
    ...
    /// 
    /// </summary>
    public class ClassSQLInfoEnumerator
    {
        ...
        [DllImport("odbc32.dll")]
        private static extern short SQLSetEnvAttr(IntPtr environmentHandle, int attribute, IntPtr valuePtr,
            int stringLength);
        ...
        private static extern short SQLBrowseConnect(IntPtr handleConnection, StringBuilder inConnection,
            short stringLength, StringBuilder outConnection, short bufferLength, out short stringLength2Ptr);
        ...
        {
            return RetrieveInformation(SQL_DRIVER_STR + ";SERVER=" + m_SQLServer + ";UID=" + m_Username + ";PWD=" +
                                       m_Password);
        }
        ...
        {
            var m_environmentHandle = IntPtr.Zero;
            var m_connectionHandle = IntPtr.Zero;
            var inConnection = new StringBuilder(InputParam);
            var stringLength = (short)inConnection.Length;
            var outConnection = new StringBuilder(DEFAULT_RESULT_SIZE);
            short stringLength2Ptr = 0;

            try
            {
                if (SQL_SUCCESS == SQLAllocHandle(SQL_HANDLE_ENV, m_environmentHandle, out m_environmentHandle))
                    if (SQL_SUCCESS == SQLSetEnvAttr(m_environmentHandle, SQL_ATTR_ODBC_VERSION, (IntPtr)SQL_OV_ODBC3,
                            0))
                        if (SQL_SUCCESS == SQLAllocHandle(SQL_HANDLE_DBC, m_environmentHandle, out m_connectionHandle))
                            if (SQL_NEED_DATA == SQLBrowseConnect(m_connectionHandle, inConnection, stringLength,
        ...
        {
            var m_Start = outConnection.IndexOf(START_STR) + 1;
            var m_lenString = outConnection.IndexOf(END_STR) - m_Start;
